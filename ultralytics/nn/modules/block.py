...
class BottleneckDilated(nn.Module):
    """Standard bottleneck with dilated convolutional layers for increased receptive field."""
    ...
    def __init__(
        ...
        Args:
            ...
            k (tuple): Kernel sizes for convolutions.
            e (float): Expansion ratio.
            d (int): Dilation rate for the second convolution.
        """
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, k[0], 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply dilated bottleneck with optional shortcut connection."""
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))


class C3k2Dilated(C2f):
    """CSP Bottleneck with 2 convolutions using dilated convolutions for larger receptive field."""
    ...
    def __init__(
        ...
        Args:
            ...
            g (int): Groups for convolutions.
            shortcut (bool): Whether to use shortcut connections.
            d (int): Dilation rate for bottleneck convolutions.
        """
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            BottleneckDilated(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0, d=d)
            for _ in range(n)
        )
