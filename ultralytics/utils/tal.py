...
class SizeAdaptiveAssigner(TaskAlignedAssigner):
    """TaskAlignedAssigner with size-adaptive one-to-one cardinality for tiny objects.

    Extends TaskAlignedAssigner to compute per-GT topk2 values based on object spatial
    extent relative to the feature pyramid stride. Tiny objects (minimum dimension below
    stride_val) receive additional one-to-one positives proportional to their size deficit,
    resolving the single-sample sparsity bottleneck in NMS-free end-to-end training
    that afflicts YOLO26 architectures on dense small-object datasets.

    Attributes:
        max_o2o (int): Maximum per-GT one-to-one positive anchors for tiny objects.
    """
    ...
    def __init__(
        ...
        topk2=None,
        max_o2o: int = 4,
        adaptive_topk2: bool = True,
    ):
        """Initialize SizeAdaptiveAssigner with adaptive topk2 support.

        Args:
            max_o2o (int): Maximum per-GT one-to-one positives for tiny objects (default 4).
            adaptive_topk2 (bool): Whether to enable adaptive topk2 behavior.
        """
        super().__init__(topk, num_classes, alpha, beta, stride, eps, topk2)
        self.max_o2o = max_o2o
        self.adaptive_topk2 = adaptive_topk2

    def compute_adaptive_topk2(self, gt_bboxes, mask_gt):
        """Compute per-GT adaptive topk2 values based on object size.

        Objects whose minimum dimension is smaller than stride_val receive
        additional positive anchors in the one-to-one assignment proportional
        to how undersized they are relative to stride_val. The per-GT adaptive
        topk2 is capped at max_o2o to preserve NMS-free decoding stability.

        Args:
            gt_bboxes (torch.Tensor): Ground truth boxes, shape (b, n_max_boxes, 4) in xyxy format.
            mask_gt (torch.Tensor): Valid GT mask, shape (b, n_max_boxes, 1).

        Returns:
            adaptive_k (torch.Tensor): Per-GT adaptive topk2 values, shape (b, n_max_boxes).
        """
        gt_xywh = xyxy2xywh(gt_bboxes)
        b, n_max, _ = gt_xywh.shape
        device = gt_xywh.device

        min_dim = gt_xywh[..., 2:4].min(-1)[0]  # (b, n_max) — smaller of w, h

        is_tiny = (min_dim < self.stride_val) & mask_gt.squeeze(-1).bool()

        scale = (self.stride_val / min_dim.clamp(min=1)).clamp(1, self.max_o2o)
        adaptive_k = torch.where(
            is_tiny,
            scale.ceil().int(),
            torch.ones_like(scale, dtype=torch.int, device=device),
        )
        return adaptive_k  # (b, n_max)

    def select_highest_overlaps(self, mask_pos, overlaps, n_max_boxes, align_metric):
        """Override to support per-GT adaptive topk2 filtering.

        Preserves multi-GT conflict resolution from the parent class,
        then applies size-adaptive topk2 pruning instead of fixed topk2=1
        when self.adaptive_topk2 is active.
        """
        fg_mask = mask_pos.sum(-2)
        if fg_mask.max() > 1:  # multi-GT conflict resolution
        ...
        if self.topk2 != self.topk:
            ...
            b, n_max, na = align_metric.shape

            mask_gt_valid = mask_pos.sum(-1, keepdim=True) > 0  # (b, n_max, 1)
            gt_bboxes_dummy = torch.zeros(b, n_max, 4, device=align_metric.device, dtype=align_metric.dtype)
            adaptive_k = self.compute_adaptive_topk2(gt_bboxes_dummy, mask_gt_valid)

            new_mask_pos = torch.zeros_like(mask_pos)
            for gt_idx in range(n_max):
                for bi in range(b):
                    k_i = int(adaptive_k[bi, gt_idx].item())
                    valid_mask = mask_gt_valid[bi, gt_idx, 0]
                    if k_i > 0 and valid_mask:
                        num_candidates = align_metric[bi, gt_idx].nonzero().numel()
                        k_actual = min(k_i, num_candidates)
                        if k_actual > 0:
                            _, topk_idx = torch.topk(align_metric[bi, gt_idx], k_actual, dim=-1, largest=True)
                            new_mask_pos[bi, gt_idx].scatter_(0, topk_idx, 1.0)

            mask_pos = new_mask_pos.float()
            fg_mask = mask_pos.sum(-2)
